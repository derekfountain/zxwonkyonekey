;; This file was originally sourced from the Z88DK project and
;; hence is licenced under the Clarified Artistic License.
;;
;; It was originally written by the same author as ZX Wonky One Key
;; and the original copyright therefore remains.
;;
;; (C) Copyright Derek Fountain 2018
	
SECTION rodata_user

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000

PUBLIC _runner_right_f1
._runner_right_f1
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00010000
	defb @00010000
	defb @00000000
	defb @00100000
	defb @00011000

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_right_f2
._runner_right_f2
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00100000
	defb @00100000
	defb @00000010
	defb @00000100
	defb @00110000

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_right_f3
._runner_right_f3
	defb @00000000
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00100000
	defb @01000000
	defb @00000000
	defb @01100110

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_right_f4
._runner_right_f4
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00010000
	defb @00100000
	defb @00000000
	defb @01000000
	defb @01001100

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_right_f5
._runner_right_f5
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00001000
	defb @00010000
	defb @00000000
	defb @00100000
	defb @00011000

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
		
PUBLIC _runner_right_f6
._runner_right_f6
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00001000
	defb @00001000
	defb @00000010
	defb @00000100
	defb @00110000

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
		
PUBLIC _runner_right_f7
._runner_right_f7
	defb @00000000
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00001000
	defb @00000100
	defb @00000000
	defb @01100110

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
		
PUBLIC _runner_right_f8
._runner_right_f8
	defb @00011000
	defb @00011110
	defb @00000000
	defb @00010000
	defb @00001000
	defb @00000000
	defb @01000000
	defb @01001100

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_left_f1
._runner_left_f1
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00001000
	defb @00010000
	defb @00000000
	defb @00000010
	defb @00110010

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_left_f2
._runner_left_f2
	defb @00000000
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00010000
	defb @00100000
	defb @00000000
	defb @01100110

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_left_f3
._runner_left_f3
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00010000
	defb @00010000
	defb @01000000
	defb @00100000
	defb @00001100

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_left_f4
._runner_left_f4
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00010000
	defb @00001000
	defb @00000000
	defb @00000100
	defb @00011000

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	
PUBLIC _runner_left_f5
._runner_left_f5
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00001000
	defb @00000100
	defb @00000000
	defb @00000010
	defb @00110010

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
		
PUBLIC _runner_left_f6
._runner_left_f6
	defb @00000000
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00000100
	defb @00000010
	defb @00000000
	defb @01100110

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
		
PUBLIC _runner_left_f7
._runner_left_f7
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00000100
	defb @00000100
	defb @01000000
	defb @00100000
	defb @00001100

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
		
PUBLIC _runner_left_f8
._runner_left_f8
	defb @00011000
	defb @01111000
	defb @00000000
	defb @00001000
	defb @00001000
	defb @00000000
	defb @00000100
	defb @00011000

	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	defb @00000000
	