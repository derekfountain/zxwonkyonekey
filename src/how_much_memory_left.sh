...
#  grep -P '__\w+size' wonky.map | grep -v \$0000

CODE_SIZE=$(perl -ne '/^__code_compiler_size\s+=\s+\$(\w+)/ && print hex($1)' < wonky.map)

echo -n "\nCompiled code size is       "
printf "%5d  " $CODE_SIZE
printf "(0x%04X) bytes\n" $CODE_SIZE

DATA_SIZE=$(perl -ne '/^__data_compiler_size\s+=\s+\$(\w+)/ && print hex($1)' < wonky.map)

echo -n "Compiled data size is       "
printf "%5d  " $DATA_SIZE
printf "(0x%04X) bytes\n" $DATA_SIZE

RODATA_USER_SIZE=$(perl -ne '/^__rodata_user_size\s+=\s+\$(\w+)/ && print hex($1)' < wonky.map)
